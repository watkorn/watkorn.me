...
**The Thai Leading Rule.** Thai stacks tone marks and vowels above and below the line, so under `:lang(th)` body text gets line-height 1.8, headings 1.35 with no negative tracking, and the Start/Select labels get extra room under the tilted pill. Short Thai phrases that must not split use a word joiner (U+2060), e.g. "หา flag ให้⁠เจอ.". Thai glyphs in the terminal fall back to Mali (mono fonts have none).
